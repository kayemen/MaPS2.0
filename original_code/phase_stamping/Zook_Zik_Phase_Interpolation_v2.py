# -*- coding: utf-8 -*-
"""
Created on Fri Aug 19 18:36:03 2016
...
###
### v2 -This version, takes frames at a 4X frame rate and then it takes the avg of 3 our of every 4 frames to find 
### averaged frames at 1X frame rate. Then it interpolates these frames to a 4X frame rate to create average interpolated frames.
### Then it uses these average interpolated frames to find the representative heartbeat .Then it phase stamps the avg frames 
### to the representative frames found from the average interpolated frames. Then it also creates representative frames from 
### the original frames by just chosing the frames with the same frame number as the representative frames 
### did in the avg interpolated frames. It assumes that the avg interpolated frames and the original frames are essentially the same. 
### Then it phase stamps the avg frames using the original frames as well. 
###############################################################################
###############################################################################
#%%
...
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
print('%%% v2 -This version, takes frames at a 4X frame rate and then it takes the avg of 3 our of every 4 frames to find  %%%') 
print('%%% averaged frames at 1X frame rate. Then it interpolates these frames to a 4X frame rate to create average interpolated frames. %%%')
print('%%% Then it uses these average interpolated frames to find the representative heartbeat .Then it phase stamps the avg frames %%%') 
print('%%% to the representative frames found from the average interpolated frames. Then it also creates representative frames from%%%')
print('%%% the original frames by just chosing the frames with the same frame number as the representative frames %%%') 
print('%%% did in the avg interpolated frames. It assumes that the avg interpolated frames and the original frames are essentially the same.  %%%')
print('%%% Then it phase stamps the avg frames using the original frames as well.   %%%')
print('%%% %%%')
...
location='F:/160719_PhaseStamping_PhaseSupersamplingTest/PhaseStamping_Supersampling_480fps_54hpf_50Kframes_copy2'
...
slope_pstamp_wav[slope_pstamp_wav <-100] = 10
slope_pstamp_avg_interpolated[slope_pstamp_avg_interpolated <-100] =10
