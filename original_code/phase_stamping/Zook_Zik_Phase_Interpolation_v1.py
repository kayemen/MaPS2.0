## -*- coding: utf-8 -*-
#"""
#Created on Thu Aug 11 17:31:40 2016
#
#@author: smadaan
#"""
...
###
### v1 -This version, takes frames at a 4X frame rate and uses these frames to find the representative frames. 
### Then it takes the avg of 3 our of every 4 frames to find averaged frames at 1X frame rate. Then it interpolates these frames
### to a 4X frame rate to create average interpolated frames. Then it phase stamps the avg frames to the representative frames found 
### from the original frames. Then it also creates representative frames from the avg interpolated frames by just chosing the frames
### with the same original frame number as the representative frames did in the original frames. It assumes that the avg interpolated
### frames and the original frames are essentially the same. Then it phase stamps the avg frames using the avg interpolated frames as well. 
###############################################################################
###############################################################################
...
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
print('%%% v1 -This version, takes frames at a 4X frame rate and uses these frames to find the representative frames. %%%') 
print('%%% Then it takes the avg of 3 our of every 4 frames to find averaged frames at 1X frame rate. Then it interpolates these frames %%%')
print('%%% to a 4X frame rate to create average interpolated frames. Then it phase stamps the avg frames to the representative frames found %%%') 
print('%%% from the original frames. Then it also creates representative frames from the avg interpolated frames by just chosing the frames %%%')
print('%%% with the same original frame number as the representative frames did in the original frames. It assumes that the avg interpolated %%%') 
print('%%% frames and the original frames are essentially the same. Then it phase stamps the avg frames using the avg interpolated frames as well. %%%')
...
location='F:/160719_PhaseStamping_PhaseSupersamplingTest/PhaseStamping_Supersampling_480fps_54hpf_50Kframes_copy1'
...
#for i in np.arange(8,9):
    print(i)
    A1 = importtiff(newfolder_wavelet,i,prefix=newname,index_start_number=index_start_number_BF,num_digits=num_digits_BF) 
...
#        SSIM = np.array([])
        for j in np.arange(int(start_frame),int(goodzookendframenumber_for_numrepframes[num_zooks_for_numrepframes-1]+1)):
            A = importtiff(newfolder_wavelet,j,prefix=newname,index_start_number=index_start_number_BF,num_digits=num_digits_BF) 
...
start_frame = 0
sizeA1 = np.asarray(np.shape(importtiff(newfolder_wavelet,0,prefix=newname,index_start_number=index_start_number_BF,num_digits=num_digits_BF)))
...
#    for k in np.arange(start_frame,goodzookendframenumber[2-1]+1):
        ...
        if(k>=i):
            Z1 = importtiff(newfolder_wavelet,int(k),prefix=newname,index_start_number=index_start_number_BF,num_digits=num_digits_BF)
            Z2 = importtiff(newfolder_wavelet,int(i),prefix=newname,index_start_number=index_start_number_BF,num_digits=num_digits_BF)
